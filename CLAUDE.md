...
- **Repo:** `git@github.com:purpleicecube/AidenIWO2.git` — branch **`iwo3/main`**, upstream `origin/iwo3/main`. (Verified 2026-09-05, Loop Omicron ο.8. The previous entry, `purpleicecube/AidenIWO-or-TIB` branch `main`, named neither the actual remote nor the actual branch: `main` in this repository is the IWO2 line, checked out in the separate `/home/virgina/VS_AIDEN_IWO2` worktree. Description corrected; the Git remote itself was NOT changed.)
...
| 2026-05-17 | **BUG-070 — Workspace folder grid + toolbar (sort + search).** Operator-reported UX gap: the Workspace Outputs folder (full of multi-attempt branded chain artifacts with near-duplicate names) had no way to find the latest render. `views/workspace.py::_render_grid` sorted both folders and files alphabetically by name only; the caller pre-sorted alphabetically too. No search input, no sort dropdown, no toolbar. Pure UI gap — `routes/workspace.py` already returned `created_at` + `updated_at` per row. Added a per-folder toolbar above the grid: search input (case-insensitive substring; live caption `Showing X of Y item(s) matching '…'` when active; friendly info banner on zero hits) + 6-option sort dropdown (`Newest first (created)` default + `Oldest first (created)` + `Recently modified` + `Name A→Z` + `Name Z→A` + `Type`). Folders stay above files (file-explorer convention) but within each group respect the chosen sort. Folders use the same vocabulary; "Type" falls back to alpha for folders (no mime_type). Toolbar state keyed per folder (`ws-search-{folder_id}` / `ws-sort-{folder_id}`) so per-folder context survives navigation between folders. Default changed from alphabetical → Newest first because the Outputs folder fills faster than operators can alphabetize. ~75 lines added in `views/workspace.py` (2 sort-key factories + 2 sort helpers + `_render_grid_toolbar` + caller refactor). No backend / schema change. Smoke 25/25 view-import green. Full BUG-070 entry in `BUGFIX_LOG.md` § "Session — IWO3 Workspace UX (2026-05-17)". Intentional residuals: search is folder-scoped only (global search is an additive follow-on); no mime-type filter chips (sort-by-Type already groups them); toolbar shows on every folder including root (could conditionally hide on small folders, kept visible by default). Working-tree only; not yet committed. |
| 2026-05-17 | **IWO2 work-order detail-page parity uplift (Sandbox Loop α Darkmode).** Per Darrel one-shot brief: reskin the IWO3 Work Orders expanded surface to mirror the IWO2 work-order detail page as closely as Streamlit allows, with the IWO2 status badge + Quality Review banner family as the **hard requirement** plus that color/icon idiom becoming the **default semantic pattern** for important operator status callouts across IWO3. Single-file change to `apps/console-streamlit/views/work_orders.py` (851 → ~1100 lines net). Added `_DETAIL_CSS` page-scoped block with three reusable CSS families: banners (amber / red / green / sky / violet — operator-significant status callouts, IWO2 soft-tinted + 4px-left-accent idiom), badges (one variant per WO status — completed / processing / pending / blocked / awaiting / deferred / failed / cancelled / reopened, IWO2 50-shade-bg + 700-shade-text + 200-shade-border + Lucide icon), cards (props soft-gray / deliv emerald-tinted / results violet-tinted). 24 Lucide MIT-licensed SVG icons inlined via `unsafe_allow_html=True` (NOT `st.html` — DOMPurify strips SVG). 9 render helpers added: `_render_status_header` (badge banner), `_render_top_toolbar` (Run Aiden / Run next step / Render via Gamma / Audit ↗), `_render_quality_review_banner` (Darla `qa_passed`/`qa_needs_revision`/`qa_blocked` driven, BUG-068 contract), `_render_terminal_banner` (generic blocked/awaiting/failed/deferred), `_render_description_card`, `_render_properties_card`, `_render_deliverables_card`, `_render_results_card`, `_render_transition_strip`. Per-WO expanded surface now follows IWO2 idiom: badge banner → top toolbar → 2-col layout (left = Quality Review banner + terminal banner + description + recovery popovers + transition strip; right = Properties + Deliverables + Results) → existing Lifecycle / Outputs / Audit tabs preserved at bottom for full evidence detail. Status badge shows in BOTH header and Properties card so the operator sees the cue twice. **Strict scope discipline:** zero changes to `/home/virgina/VS_AIDEN_IWO2`; zero backend / schema / RBAC / audit changes; zero list-page rework (deferred per brief). Validation: AST parse clean, view-import smoke 25/25 green, full Streamlit pytest 60/0 + 5 skipped (pre-existing env-gated), `curl /work_orders` HTTP 200, all 9 helpers + 24 Lucide icons + 10 status badge entries present and well-formed. **No BFL entry** filed — parity uplift, not a named defect (per brief). BFL next-free remains BUG-071. Handback packet for CODEX at `WS024_IWO3[Branch]/05_Artifacts/CLAUDE_HANDBACK_IWO2_WORK_ORDER_DETAIL_PARITY_DARKMODE_2026-05-17.md` (11 sections + 5 review asks Q-α-1..5). Deferred next-loop candidates captured: list-page parity (held until operator request), `_DETAIL_CSS` promotion into `shell.py` (held until 2nd consumer), live Darla QA banner browser verification, toolbar per-button semantic color (Streamlit ceiling — defer unless asked). Working-tree only; not yet committed (stacks on the same uncommitted tree as BUG-070 + the v1.5.1 / Aiden Alpha v4.0.1 bump). |
| 2026-05-17 | **IWO2 work-order detail-page polish (Sandbox Loop α Darkmode, second pass — mirror of WS024 session note).** Polish pass over the parity surface shipped earlier today. Operator feedback: heading in the right direction but still messy — badge floating disconnected from title, toolbar oversized/heavy, transitions row wrapped on `awaiting_operator`, right rail too tall, page reading as six stacked boxes, duplicate state communication. Same single file rewrite of `apps/console-streamlit/views/work_orders.py` (~1100 → ~1060 lines net, reflowed). Disposition of the 6 named problems: **(1) status placement** — `_render_status_header` → `_render_summary_band` (title `<h3>` + badge + priority in one flex row + meta line beneath: ID short / Type / Created N ago / Updated N ago via new `_ts_relative` helper); subtle 1px bottom border anchors the band. **(2) toolbar** — dropped "Actions" group label, 3 buttons `type="secondary"` + 1 `type="tertiary"` (text-only), columns `[2,2,2,2,4]` so toolbar sits on left half with breathing space; dropped emoji prefixes; shorter labels. No primary blue anywhere. **(3) transitions** — `_render_transition_strip` → `_render_transitions_popover` (single `Status ▾` trigger in 3-of-12 column; opens vertical list with one short-labeled button per legal target). New `_TRANSITION_LABEL` map gives `awaiting_operator` → `Await operator`. No wraps possible. **(4) right rail** — Properties slimmed 7-8 rows → 2-3 (Submitter / Full ID / optional Full corr.); Status/Priority/ID-short/Type/Created/Updated all moved to summary band, eliminating duplicate. Padding tightened (12-14px → 10-12px), border-left 4px → 3px, card headers uppercase 0.80rem label-style. Rail visibly shorter than left column. **(5) stacked-box feel** — description card chrome removed (`_render_description_card` → `_render_description_flow`, just typography), recovery row sheds its "Recovery" label. Left column reads as flowing narrative; right rail keeps card emphasis where semantic. **(6) repeated state** — badge shown EXACTLY ONCE on the detail surface (in the summary band); collapsed-list expander label keeps `_state_chip` emoji for scan use-case (different reading context). Accepted semantic foundations preserved unchanged: muted amber/yellow/green/red/sky/violet, icon-led status language, quality-review banner, two-column direction, reusable iwo3-banner/iwo3-badge/iwo3-card family. New helpers: `_render_summary_band`, `_render_transitions_popover`, `_render_description_flow`, `_ts_relative` (stdlib `datetime` only, no new deps). Validation: AST parse clean, view-import smoke 25/25 green, full Streamlit pytest 60/0 + 5 skipped, `curl /work_orders` HTTP 200, all 12 polish helpers present, 3 prior-pass helper names cleanly removed. No BFL entry (composition polish, not a named defect). Handback at `WS024_IWO3[Branch]/05_Artifacts/CLAUDE_HANDBACK_IWO2_WORK_ORDER_DETAIL_POLISH_DARKMODE_2026-05-17.md` (10 sections + 5 CODEX asks Q-α-polish-1..5). Working-tree only; not yet committed (stacks on parity pass + BUG-070 + v1.5.1/Aiden Alpha v4.0.1 bump, all uncommitted). |
| 2026-09-05 | **IWO3 stand-up after a 3½-month idle — seven defects filed `BUG-078`..`BUG-084`, all in `BUGFIX_LOG.md`.** Runtime brought up (FastAPI :8000, Streamlit console on **:8502** — DigiFlow holds :8501, so `scripts/iwo3.sh status` reports a false green there). **BUG-078 (Critical):** `action_audit_log` partitions expired 2026-07-01, so every audited mutation had 500'd platform-wide — ~55 call sites, plus 60 vitest failures on any fresh reset. Migration 0002 shipped `ensure_audit_partition_for()` for exactly this and nothing ever called it. New `workers/audit_partition_worker.py` ensures the window at startup *before traffic* and daily; `apply-migrations.ts` does the same for the vitest lane (it never boots the API). Integration 60 → 19 failures, 0 partition errors. **BUG-079 (High):** Aiden never searched the web — measured **0/6** world-knowledge probes. Three instructions had to move; the load-bearing one was the shared memory `GROUNDING RULES` preamble saying *"use ONLY the sources below"* with no counterpart permitting a tool call to GO AND GET a source. Now **6/6**, controls unchanged, with search provenance + clickable sources in chat. **BUG-080 (High):** provider changed without its credential — Darla saved as `groq` + `OPENROUTER_API_KEY` and 401'd on every invoke. Guard now on **all four** write paths (create / update / rollback / seed loader) with an AST invariant gate that fails when a *new* write path skips it; a poisoned rollback snapshot already existed in `llm_config_versions`. **BUG-081 (Critical):** Klear's Gamma credential was destroyed on 2026-05-19 by `test_poll_registry.py` running against the **live** database — it DELETEs and rewrites `adapter_credentials` with no teardown. BUG-069 had already fixed this credential three days earlier. `tests/conftest.py` now refuses any pytest session whose DB name does not end in `_test` (`IWO3_ALLOW_NON_TEST_DB=true` overrides); the vitest lane had this guard, pytest had none. Gamma renders again end-to-end to a real PDF. **BUG-082 (Medium):** the Gamma health check ran `SELECT 1` — it counted a row instead of resolving the credential, so it read ✅ Pass throughout. **BUG-083 (High):** Aiden had **no workspace tools** at all, so "create a dated folder" became a work order and Paul produced a *"Deployment Report"* markdown file describing a folder creation; Aiden then invented `<BASE_URL>/workspace/folder/…` URLs for routes that never existed. Registry 14 → 17 (`workspace_create_folder` / `workspace_list_tree` / `workspace_locate_output`; no delete or rename by design). **Architectural finding: rules written into `AIDEN_SYSTEM_PROMPT` are dead code for any tenant with a custom prompt** — Klear's is 12,774 chars and replaces it entirely; only `_AIDEN_OUTPUT_SCHEMA_TEMPLATE` reaches every tenant. **BUG-084 (Medium):** output filing had gone flat — IWO2's `<date>/<order-slug>_<id8>` structure restored, artifacts now carry `workOrderId`, workspace deep links (`?file=<id>`), rendered markdown, `.md` + self-contained `.html` downloads. PDF/DOCX buttons deliberately **not** shipped — both renderers are stubs; the viewer says so and points at Render Gamma. **Also shipped (feature, not a bug):** catalogue-driven Sub-Agent model dropdown — live Groq (8 chat-capable of 14) / OpenRouter (430 of 431) lists, 6h TTL cache with manual refresh and stale-on-failure fallback, advisory chat-capability filter, and a hard rule that a retired or filtered model stays selectable so a dropdown can never silently re-point an agent. Aiden's own wiring untouched per operator instruction. **Verification:** 743 API + 71 console tests green; 69+ new tests; every gate validated by reintroducing its defect. **New gaps opened as CR-012..CR-019** in `ROADMAP_FEATURES_v2.2.md` (roadmap → v2.4): health checks that count rather than resolve, `sandbox_docx` stub with no fallback, prompt-block dead-code rule, Tier-1 tool assignment not honoured, 79 frozen work orders, 50 junk workspace folders, 19 pre-existing vitest failures, and Tier-1 prompt crowding (`test_routing_compliance_to_nyx` now flakes ~2 in 5). |
