...
### BUG-073: FFAI tenant missing Gamma `adapter_credentials` row on hosted — gamma_pptx WOs stuck in `processing` forever (High, hot-fix; hosted data backfill)

| Field | Detail |
| --- | --- |
| Date | 2026-05-17 |
| Severity | High (every FFAI gamma_pptx WO blocks at the Gamma handoff gate with `gate_refused:credential_missing` — never produces a Gamma deck, never completes, never returns a `gamma_url`. Klear tenant is unaffected; this is FFAI-only data drift on hosted Postgres. Same shape as the original BUG-063 from 2026-05-16 which was fixed LOCALLY at the time but never propagated to hosted.) |
| Status | Fixed (hosted data backfill); no code change required |
| Files | n/a — hosted-data-only fix; no repo change |
| Symptom | Operator on hosted Streamlit created a "5-slide Intro PPT for FF.AI" WO (`10ccca78-e472-4c73-9094-dec36a0d5565`) as `owner_ffai@dev.local` (FFAI tenant). Worker dispatched cleanly through Aiden Tier-1 (work_order_brief, 3001 tokens) + Tom Tier-2 (`tom_tier_2`, content_envelope, 1879 tokens) and created `output_packages.gamma_pptx` (`f409c500-...`) in `draft` status. WO transitioned to `processing` per state machine. Then sat there. Operator opened the Work Order detail surface ~1 min after creation and saw the WO still in `processing` with the deliverables card showing `kind=gamma_pptx status=draft` — no handoff_id, no gamma_url. |
| Root Cause | `apps/api-fastapi/adapter/dispatch.py::dispatch_gamma_for_package` requires an `adapter_credentials` row for the tenant + Gamma adapter before it will create an `output_handoffs` row + call the Gamma API. On hosted Postgres, only Klear had this row (provisioned via WS021 hosted parity execution: `credential_ref:env:GAMMA_API_KEY`, status=active, first_invocation_confirmed_at set). FFAI had ZERO rows. When the worker called `dispatch_gamma_for_package` for the FFAI package, the credential gate refused immediately with `kind=gate_refused:credential_missing` and `detail="no adapter_credentials row for adapter=gamma"` — captured in the `adapter_dispatch.failed` audit row with `stage=auto_dispatch_post_package`. No handoff row was created; the poll_worker had nothing to poll; the WO stayed in `processing` forever because the BUG-071 worker auto-complete deliberately skips `gamma_*` packages (relying on the poll_worker for completion). |
| Fix | Inserted a single `adapter_credentials` row for FFAI tenant on hosted Postgres, mirroring the Klear shape exactly: `client_id=00000000-0000-4000-8000-00000000c002`, `adapter_catalog_id=<id of adapter_key='gamma'>`, `credential_ref='credential_ref:env:GAMMA_API_KEY'` (same env var Klear uses; the hosted Railway env var `GAMMA_API_KEY` is shared across both tenants today), `status='active'`, `first_invocation_confirmed_at=now()`, `metadata={"provisioned_by": "bug_073_hotfix_backfill", "note": "..."}`. Then re-fired Gamma render on the stuck WO via `POST /work_orders/10ccca78-.../render` (the existing operator recovery endpoint). Gamma accepted at t+0s, handoff `a9a35a03-...` written with `status=submitted`. The poll_worker advanced handoff to `completed` at t+45s, the WO followed to `completed` per the existing gamma poll → WO auto-advance path. End-to-end gamma_pptx flow verified working on FFAI for the first time on hosted. |
| Verified | Pre-fix: `adapter_credentials` had 1 row total (Klear/gamma). Post-fix: 2 rows (Klear/gamma + FFAI/gamma both active + confirmed). Re-fired render returned HTTP 200 with `handoff_id=a9a35a03..., external_reference=dxjA0Edii1FnvNYCnWENx`. Polling cadence: t=15s `wo=processing, handoff=submitted` → t=30s same → t=45s `wo=completed, handoff=completed`. Operator can now refresh the Work Order detail page and see the WO in `completed` status with the Gamma URL surfaced in the deliverables card. |
| Files Changed | n/a — hosted Postgres only; no repo code change |
| Related | **Scope:** narrowest operator-visible fix for FFAI gamma_pptx flow on hosted. Same kind of fix as BUG-063 from 2026-05-16 (FFAI missing Gamma credential locally) — that one fixed local; this one fixes hosted. **Adjacent architectural concern NOT addressed:** when `dispatch_gamma_for_package` fails with `gate_refused:credential_missing` or any other terminal adapter error, the WO sits in `processing` forever because the poll_worker only advances on a successful handoff and the worker's auto-complete logic deliberately skips gamma_*. **The right systemic fix is to transition the WO to `awaiting_operator` or `blocked` with a clear operator-facing reason when adapter dispatch fails terminally** — so operators see "blocked: missing Gamma credential" instead of an indefinite `processing` status. Captured for a separate BUG (BUG-074-class). This loop only does the hosted data backfill so the WO actually completes. **Cross-tenant credential provisioning seed gap:** `IWO3_SEED_SCOPE=reference` deliberately does NOT seed `adapter_credentials` rows (per `infra/local/seed_user_passwords.py:23`-style deliberate-skip); operators must manually provision per-tenant credentials post-deploy. A future operator runbook update should call this out as a checklist item for every new tenant onboarding. |
